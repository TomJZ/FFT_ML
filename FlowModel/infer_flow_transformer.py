...
def infer(model, train_loader, eval_loader, forecast_loader, inference_loader, save_name, epoch):
    ...
    for idx, data in enumerate(inference_loader):
        si_inference, ti_inference = [data_i.to(device) for data_i in data]
        si_inference[si_inference > 10e10] = 0
        si_inference_lat_lon = si_inference[:, :, :, :, :2]
        si_inference_uv = si_inference[:, :, :, :, 2:]
    ...
    with torch.no_grad():
        ...
        # context points
        si_pred_ctx = si_inference_uv.to(device)
        # si_gt = si_eval_uv
        ...
        # save the animation
        # vis_vf_large(epoch, zi_vis_flat, zi_vis_flat, zi_vis_flat, save_name)
        ...
        # compose the 8-day forecast and 60-hrs transformer inference
        forecast_8day = torch.cat([si_inference_lat_lon, si_inference_uv], dim=-1)
        transformer_60hr = torch.cat([si_inference_lat_lon[:,:vis_len], zi_vis], dim=-1)

        torch.save(torch.cat([forecast_8day, transformer_60hr], dim=1).squeeze(0), './PredictionTensor/fore_plus_trans/forecast_8days_transformer_60hrs_predict_'+save_name+'.pth')
...
if __name__ == '__main__':
    ...
    inference_dataset = Dataset_builder(inference_data_folder, "inference", lat_min, lat_max, lon_min, lon_max)
    ...
    if mode == "Train":
        ...
        epoch = 10000
        # load model
        node_flow_model = torch.load(save_path+'_epoch'+str(epoch)+'.pth', map_location='cuda:0')["model"]
    ...
    # see if the model could overfit
        ...
        infer(node_flow_model, train_loader, eval_loader, forecast_loader, inference_loader, save_name=save_name, epoch=epoch)